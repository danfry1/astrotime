...
| REQ-TBL-008 | Leap-table files reject ambiguous duplicate metadata and non-comment trailing row data. | `rejects ambiguous duplicate metadata and trailing row garbage` |
...
| REQ-JD-002 | Two-part Julian dates preserve the exact nanosecond through every scale and every known leap boundary. | `two-part Julian dates round-trip exactly on every scale`, `two-part UTC Julian dates round-trip exactly at every leap boundary` |
