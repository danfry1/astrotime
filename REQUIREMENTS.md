...
| REQ-FMT-007 | Stray letters are reported per run, and a non-ASCII letter is literal text rather than a suspected token. | `reports stray letters per run, not merged across the tokens between them`, `treats a non-ASCII letter as literal text, since no token is one` |
